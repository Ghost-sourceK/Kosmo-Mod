...
public class LaserEffect extends AbstractGameEffect {
  private float x;
  private float y;
  private float destY;
  ...
  public LaserEffect(float x, float y, Color col, String soundKey) {
    this.img = ImageMaster.DAGGER_STREAK;
    this.x = x - MathUtils.random(320.0F, 360.0F) - this.img.packedWidth / 2.0F;
    this.destY = y;
    this.y = this.destY + MathUtils.random(-230.0F, -200.0F) * Settings.scale - this.img.packedHeight / 2.0F;
  ...
  public LaserEffect(float x, float y, Color col) {
    this(x, y, col, "CARD_EXHAUST");
  }
  
  public LaserEffect(float x, float y, float fAngle, Color col) {
    this.img = ImageMaster.DAGGER_STREAK;
    this.x = x - MathUtils.random(320.0F, 360.0F) - this.img.packedWidth / 2.0F;
    this.destY = y;
    this.y = this.destY + MathUtils.random(-230.0F, -200.0F) * Settings.scale - this.img.packedHeight / 2.0F;
  ...
  public void update() {
    if (!this.playedSound) {
      CardCrawlGame.sound.playA(this.soundKey, 1.0F);
      this.playedSound = true;
    } 
    ...
    this.scale = Interpolation.bounceIn.apply(Settings.scale * 0.4F, Settings.scale * 1.1F, this.duration / 0.4F);
  }
