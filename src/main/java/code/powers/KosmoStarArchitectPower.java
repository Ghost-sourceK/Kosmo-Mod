...
public class KosmoStarArchitectPower extends LambdaPower {
    ...
    public void updateDescription() {
        if (upgradedStacks > 0 && normalStacks > 0) {
            this.description = "At the start of your turn, add " + upgradedStacks + " Dormant Star+ and " + normalStacks + " Dormant Star into the Exhaust pile.";
        } else if (upgradedStacks > 0) {
            this.description = "At the start of your turn, add " + upgradedStacks + " Dormant Star+ into the Exhaust pile.";
        } else {
            this.description = "At the start of your turn, add " + normalStacks + " Dormant Star into the Exhaust pile.";
        }
    }
    ...
    public void atStartOfTurn() {
        if (upgradedStacks > 0) {
            ...
            c.upgrade();
            addToBot(new MakeTempCardInExhaustAction(c, upgradedStacks, 0.55F));
        }
        if (normalStacks > 0) {
            addToBot(new MakeTempCardInExhaustAction(new DormantStar(), normalStacks, 0.55F));
        }
    }
